...
\subsection{Scenario Planning and Explanation}
...
The Dynamic Business Performance Forecaster is composed of four modular components:
data ingestion and preprocessing, model training and inference, natural language explanation generation, and the interactive dashboard interface.
...
% \end{center}
%\end{figure}
The Dynamic Business Performance Forecaster was evaluated using a retail store sales
dataset containing daily records across multiple store locations. The dataset included five
...
\begin{table}[b]
  ...
  \textbf{Metric} & \textbf{Description} & \textbf{Value} \\
    \hline
    RMSE  & Root Mean Squared Error      & 19.80  \\
    MAE   & Mean Absolute Error          & 12.73  \\
    MAPE  & Mean Absolute Percentage Error & 0.50\% \\
    R2 & Coefficient of Determination & 0.983  \\
    \hline
  \end{tabular}
  \centering
  \caption{XGBoost Model Evaluation Metrics on Held-Out Test Set}
\end{table}
The model achieved an R2 of 0.983, indicating that approximately 98.3\% of the
variance in store sales is explained by the model. The low MAPE of 0.50\% confirms
that predictions deviate from actual sales values by less than one percent on average,
which represents strong forecasting accuracy for a general-purpose, domain-agnostic
system. The RMSE of 19.80 and MAE of 12.73 indicate the typical size of prediction errors in the same units as the sales values themselves. The difference between these two metrics suggests that the model's errors are fairly stable, with very few extreme mistakes that would make the RMSE much larger than the MAE.
