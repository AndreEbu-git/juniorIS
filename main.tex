...
Instead of being specific to a particular domain, the system is designed to accept general-purpose structured tabular data. For evaluation, the system is tested on a publicly available
...
\appendix
\section{Addendum: Use of Generative Artificial Intelligence}
Generative artificial intelligence tools were used selectively to help in the planning and writing of this Independent Study project in supporting capacity. Their application was strictly confined to 
brainstorming implementation options, explaining technical terminology, debugging support, light interface modifications, and editing individual sentences to enhance clarity and grammar (Grammarly). The tools were not used to generate the primary research outcome, produce any experimental outcomes/results, or create the forecast model method that is introduced in this thesis.
During the software development process, ChatGPT and OpenAI Codex were utilized as interactive aids when building and improving the Dynamic Business Performance Forecaster. They were consulted in the same way as other forms of technical literature, forums, or code samples would have been. The prompts asked were brief, focused technical inquiries regarding Streamlit's function, debugging in Python, time series preprocessing, feature engineering, and API integrating.

More specifically, AI was utilized for optimizing the user interface, enhancing the preprocessing pipeline for uploaded business datasets, and resolving issues related to the machine learning workflow. This included clarifying how to handle date columns and numeric-type columns, and how to avoid errors when datetime fields appeared alongside the numeric variables.
Artificial intelligence was also used to implement the natural-language explanation module that connects the system to an external large language model through an API key.
Furthermore, AI was used as a resource in interpreting Python error messages, understanding their causes, and suggesting methods for overcoming the problem. Some examples include resolving problems with loading environment variables from the local \texttt{.env} file. In each case, suggested solutions were reviewed, tested, revised, and manually integrated rather than being copied into the system without evaluation.

On the writing side, AI in this paper was confined to the editing of sentences and making some explanations clearer. Grammarly and Chat GPT handled issues related to wording, grammar, and readability, but not the content itself, these revision tools were only viewed as editorial services.

All the AI recommendations made were taken on an advisory basis. Every code modification, design decision, explanation, and piece of text adopted in this project was carefully scrutinized, revised where necessary, and verified before being accepted.  The forecasting architecture, implementation decisions, experimental evaluation, and interpretations presented in this thesis remain the responsibility of the author. The author therefore accepts full responsibility for the correctness, design, implementation, and written presentation of all components of this project.
